...
export async function AddWeeklyExpenses(req: Request, res: Response) {
  ...
  if (
    ...
    return res.status(400).send({ res: "Todos os campos são obrigatórios." });
  }
  ...
  try {
    ...
    if (!userBudget) {
      return res
        .status(404)
        .send({ res: "Orçamento não encontrado para o usuário especificado." });
    }
    ...
    if (!week) {
      return res.status(404).send({ res: "Semana não encontrada." });
    }
    ...
    console.error({ res: "Erro ao adicionar despesa:", error });
    res.status(500).send({ res: "Erro interno do servidor." });
  }
}
